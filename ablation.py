...
-----------------------------------------------------------------------
Modes:
1. BERT Only (Text Features)
2. GAT Only (Network Structure + Metadata)
3. Fusion (Composite)
"""
...
RESULTS_FILE = DATA_DIR / "ablation_metrics_summary.json"
...
class RiskModel(nn.Module):
    def __init__(self, mode, node_dim, text_dim=768):
        super().__init__()
        self.mode = mode
        
        # --- GAT Components (Used in 'gat' and 'fusion') ---
        if self.mode in ['gat', 'fusion']:
            self.gat1 = GATConv(node_dim, 32, heads=4, dropout=0.4)
            self.gat2 = GATConv(32*4, 32, heads=1, concat=False, dropout=0.4)
            gat_out_dim = 32
        else:
            gat_out_dim = 0

        # --- BERT Components (Used in 'bert' and 'fusion') ---
        if self.mode in ['bert', 'fusion']:
            bert_dim = text_dim
        else:
            bert_dim = 0
            
        # --- Classifier Head ---
        # Input to classifier depends on mode
        combined_dim = bert_dim + gat_out_dim
    ...
    def forward(self, txt, x, edge_index):
        ...
        if self.mode in ['gat', 'fusion']:
            ...
            g_out = self.gat2(g, edge_index)
            features.append(g_out)
...
def train_and_evaluate(mode, nodes, text_embs, node_feats, labels, edge_index, node_degrees):
    print(f"\n[{mode.upper()}] Starting {args.n_splits}-Fold CV...")
    ...
    text_embs_t = text_embs.to(DEVICE)
    node_degrees_t = node_degrees.to(DEVICE)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels.numpy())):
        ...
        # If using GAT, filter isolated nodes during eval to be fair
        if mode in ['gat', 'fusion']:
            connected_nodes_mask = (node_degrees_t > 0)
            final_eval_mask = val_bool_mask & connected_nodes_mask
            if final_eval_mask.sum() < 10: final_eval_mask = val_bool_mask
        else:
            final_eval_mask = val_bool_mask # BERT only doesn't care about isolation
        ...
        model = RiskModel(mode=mode, node_dim=node_feats.shape[1]).to(DEVICE)
        ...
        for epoch in range(args.epochs):
            model.train()
            opt.zero_grad()
            logits = model(text_embs_t, node_feats_t, edge_index_t)
...
def main():
    ...
    # Embeddings generated once, reused for BERT and Fusion modes
    text_embs = embedder.get_embeddings(edges, uid2idx, len(nodes))
    
    # Define Experiments
    experiments = ['bert', 'gat', 'fusion']
    final_results = {}
    
    print(f"\nTraining on {DEVICE}...")
    
    for mode in experiments:
        print(f"\n{'='*20} EXPERIMENT: {mode.upper()} {'='*20}")
        res = train_and_evaluate(mode, nodes, text_embs, node_feats, labels, edge_index, node_degrees)
        final_results[mode] = res
        print(f"RESULT {mode.upper()}: F1 {res['mean_macro_f1']:.4f} | AP {res['mean_avg_precision']:.4f}")
