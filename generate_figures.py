...
def plot_architecture():
    """Generates a schematic block diagram of the Fusion Model."""
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.axis('off')
    
    boxes = {
        "Tweets": (0.1, 0.7, "User Tweets\n(Input)"),
        "BERT": (0.3, 0.7, "DistilBERT\n(Text Branch)"),
        "TextEmb": (0.5, 0.7, "Text Emb.\n(768-d)"),
        
        "Edges": (0.1, 0.3, "Graph Edges\n(Input)"),
        "GAT": (0.3, 0.3, "GAT\n(Graph Branch)"),
        "GraphEmb": (0.5, 0.3, "Graph Emb.\n(32-d)"),
        
        "Concat": (0.7, 0.5, "Concatenation\n(Fusion Layer)"),
        "MLP": (0.85, 0.5, "MLP Classifier\n(Output)")
    }
    
    for name, (x, y, label) in boxes.items():
        box_props = dict(boxstyle="round,pad=0.5", fc="white", ec="#333333", lw=2)
        ax.text(x, y, label, ha="center", va="center", size=10, bbox=box_props)

    arrows = [
        ("Tweets", "BERT"), ("BERT", "TextEmb"), ("TextEmb", "Concat"),
        ("Edges", "GAT"), ("GAT", "GraphEmb"), ("GraphEmb", "Concat"),
        ("Concat", "MLP")
    ]
    ...
    for start, end in arrows:
        ...
        end_pos = boxes[end]
        ax.annotate("", xy=(end_pos[0]-0.06, end_pos[1]), xytext=(start_pos[0]+0.06, start_pos[1]),
                    arrowprops=dict(arrowstyle="->", lw=2, color="#555555"))
        
    ax.text(0.3, 0.5, "Transductive\nContext", ha="center", va="center", 
            color="gray", style="italic", fontsize=9, bbox=dict(fc="#f0f0f0", ec="none"))

    plt.title("Multi-Modal Fusion Architecture", fontsize=14, pad=20)
    plt.tight_layout()
...
def plot_comparative_roc():
    ...
    plt.tight_layout()
    plt.savefig("roc_curve.png", dpi=300) # Overwriting the single curve with this better one
...
if __name__ == "__main__":
    plot_architecture()
    plot_scalability()
    plot_comparative_roc() # This now generates the multi-line plot
